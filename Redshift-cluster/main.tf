...
resource "aws_redshift_cluster" "cluster" {
  cluster_identifier = var.redshift.cluster_name

  cluster_version = var.redshift.cluster_version

  database_name = var.redshift.initial_database_name
  ...
  cluster_subnet_group_name = aws_redshift_subnet_group.subnet-group.name

  publicly_accessible = false

  multi_az        = startswith(var.redshift.node_type, "ra3")
  ...
  master_password_secret_kms_key_id = aws_kms_key.redshift-cmk.id
...
# ========================================================
resource "aws_redshift_subnet_group" "subnet-group" {
...
data "aws_secretsmanager_secret" "redshift_secrets" {
  arn = aws_redshift_cluster.cluster.master_password_secret_arn
}
