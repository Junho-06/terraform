output "rds" {
  value = {
    Aurora_MySQL = {
      ...
      Passwrod = var.db_engine.create_aurora_mysql_cluster == true && var.aurora-mysql.manage_master_user_password == true ? nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.mysql_secrets_current[0].secret_string)["password"]) : var.aurora-mysql.manage_master_user_password == false ? try(var.aurora-mysql.master_password, null) : null
    ...
    Aurora_PostgreSQL = {
      ...
      Passwrod = var.db_engine.create_aurora_postgres_cluster == true && var.aurora-postgres.manage_master_user_password == true ? nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.postgres_secrets_current[0].secret_string)["password"]) : var.aurora-postgres.manage_master_user_password == false ? try(var.aurora-postgres.master_password, null) : null
