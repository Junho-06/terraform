output "bastion" {
  value = {
    ...
    SSH_Connnect_Command = "ssh -i ${var.key}.pem ${startswith(var.ec2.ec2_ami, "Amazon") ? "ec2-user" : "ubuntu"}@${var.ec2.map_eip_to_bastion == true ? aws_eip.bastion-eip[0].public_ip : aws_instance.bastion.public_ip}"
  }
}
